...
不要根据旧 agent 的口头报告判断完成度，以当前工作树和实际请求结果为准。本轮内容已于 2026-09-22 提交并推送到 `origin/main`（见 `IMPLEMENTATION.md`）；后续修改仍不要 reset、checkout 或清理未跟踪文件，提交与推送需用户明确要求。
...
- 2026-09-22 已分批提交并推送；此后是否提交/推送由用户决定。最终交付要说明实际改动文件、命令结果、浏览器路径、截图和剩余延后项。

## 最短继续顺序

1. 先读 `IMPLEMENTATION.md` 最新记录与当前代码，避免重复实现 I01/I02；
2. 未知评分契约与 legacy 去假数据/窄屏收尾已完成，远程 CI 也已验证通过（2026-09-22）；剩余明确遗留是自动化规则 CRUD、数据源配置/OAuth、日报生成与通知发送；
3. 按本轮实际改动做既有回归与隔离 HTTP/浏览器验证，不修改用户主库；
4. 扩展规则配置、日报或通知前，先确定下一轮功能范围，不把演示标记移除当作实现；
5. 更新证据、剩余项及服务清理情况；提交或推送前先取得用户确认。
